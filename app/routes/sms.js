     ...
     Contact.find({user:req.user}, function(err, contacts){
       ...
       if(contacts.length==0){
        ...
        Balance.find({user:req.user}, function(err, balance){
          ...
          }else if(message.length>160 && message.length < 321){
            var b = bal/2;
            if(contacts.length>b){
                           ...
                           if ( recipients.length > 0 ) {
                               for (var i = 0; i < recipients.length; ++i ) {
                                   ...
                                   if(status = "Success"){
                                     ...
                                     var balance = new Balance();
                                     balance._id = bal_id;
                                     balance.update({tokens:tokn}, function(err, token){
                                     if(err) return err;
                                     console.log(token);
                                       })
...
app.get('/send/:user_id/:bulk_id', function(req, res){

  var user_id = req.params.user_id;
  var bulk_id = req.params.bulk_id;
  Bulk.find({_id:bulk_id}, function(err, bulk){
    if(err) return err;
